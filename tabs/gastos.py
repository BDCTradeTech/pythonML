...
def _cruzar_ventas_reporte_vs_bd(
    ...
    anuladas_ok = sum(1 for a in anuladas if a["_marcada_ok"])
    # Las anuladas correctamente marcadas como canceladas en BD son operación normal,
    # no un error de cruce: cuentan como "procesadas OK" igual que las cruzadas.
    salud = ((len(cruzadas_ok) + anuladas_ok) / total_reporte * 100) if total_reporte else 0.0

    return {
        "cruzadas_ok": cruzadas_ok, "solo_reporte": solo_reporte, "solo_bd": solo_bd,
        "con_diferencias": con_diferencias, "anuladas": anuladas,
        "anuladas_ok_count": anuladas_ok,
...
def _render_consolidado_html(resultado: dict) -> str:
    ...
    def _fila_flujo(concepto, monto, fuentes=None, nota=None, prefijo: bool = True) -> str:
        _pre = ""
        if prefijo and monto is not None and monto != 0:
            _pre = "(–) " if monto < 0 else "(+) "
        badges = render_fuente_badges(fuentes)
        _nota_html = (
            f'<div style="font-size:10px;color:{_GRAY};font-style:italic;'
            f'margin-top:4px;line-height:1.4;text-align:right">{nota}</div>'
            if nota else ""
        )
        return (
            '<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 14px;'
            'border-bottom:1px solid #f5f5f5;font-size:12px">'
            f'<span style="color:#555">{_pre}{concepto}</span>'
            '<span style="display:flex;flex-direction:column;align-items:flex-end">'
            '<span style="display:flex;align-items:center;gap:6px">'
            f'<span style="color:{_VALOR_NEUTRO};font-variant-numeric:tabular-nums">{_money_flujo(monto)}</span>'
            f'{badges}</span>'
            f'{_nota_html}'
            '</span></div>'
        )
    ...
    s.append(_fila_flujo("Ingresos brutos por ventas", flujo["ingresos_brutos"], fuentes=["repo"], prefijo=False))
...
def _fila_cruce_categoria(label: str, count: int, icono: str, icon_bg: str, icon_color: str, descripcion: str) -> str:
    op = "opacity:0.55" if count == 0 else ""
    estado = (
        '<i class="ti ti-circle-check-filled" style="color:#3B6D11;font-size:15px"></i>'
        if count > 0 else '<span style="color:#B0B0B0">—</span>'
    )
    return (
        f'<tr style="{op}">'
        '<td style="padding:8px 14px;border-bottom:1px solid #f0f0f0;text-align:left">'
        '<span style="display:inline-flex;align-items:center;gap:8px">'
        f'<span style="width:22px;height:22px;flex-shrink:0;border-radius:4px;'
        f'background:{icon_bg};color:{icon_color};display:inline-flex;align-items:center;'
        f'justify-content:center"><i class="ti {icono}" style="font-size:12px"></i></span>'
        f'<span style="font-size:12px;color:#333">{label}</span></span></td>'
        '<td style="padding:8px 14px;border-bottom:1px solid #f0f0f0;text-align:right;'
        f'font-size:14px;font-weight:700;font-variant-numeric:tabular-nums;color:{icon_color}">'
        f'{_ar_num(count)}</td>'
        '<td style="padding:8px 14px;border-bottom:1px solid #f0f0f0;text-align:left;'
        f'font-size:11px;color:#9E9E9E">{descripcion}</td>'
        f'<td style="padding:8px 14px;border-bottom:1px solid #f0f0f0;text-align:center">{estado}</td>'
        '</tr>'
    )
...
def _render_seccion_cruce_ventas_html(resultado: dict) -> str:
    ...
    salud = cr.get("salud_pct", 0.0)
    if salud >= 95:
        hero_bg1, hero_bg2, hero_border = "#EAF3DE", "#F7F9F2", "#C5E0A5"
    ...
    _hero_html = (
        f'<div style="padding:12px 14px;background:linear-gradient(135deg,{hero_bg1},{hero_bg2});'
        f'border-bottom:0.5px solid {hero_border};display:flex;align-items:center;gap:14px">'
        f'<i class="ti {hero_icon}" style="font-size:24px;color:{hero_icon_color}"></i>'
        '<span>'
        f'<span style="display:block;font-size:10px;font-weight:700;text-transform:uppercase;'
        f'letter-spacing:0.04em;color:{hero_title_color}">Salud del período</span>'
        f'<span style="font-size:22px;font-weight:700;color:{hero_value_color}">{salud_str}</span>'
        '</span>'
        f'<span style="font-size:11px;color:{hero_title_color}">'
        f'{_ar_num(procesadas_ok)} de {_ar_num(total_reporte)} ventas procesadas correctamente</span>'
        '</div>'
    )

    # --- Tabla de detalle del cruce ---
    categorias = [
        ("Concretadas", len(cruzadas_ok), "ti-check", "#EAF3DE", "#3B6D11",
         "Coinciden ML y tu BD, montos correctos"),
        ("Canceladas", len(anuladas), "ti-refresh-alert", "#EAF3DE", "#3B6D11",
         "Devueltas o canceladas, marcadas OK en la BD"),
        ("Solo en Reporte ML", len(solo_reporte), "ti-alert-triangle", "#FBF1DC", "#7A5A0E",
         "Ventas que ML facturó pero faltan en tu BD"),
        ("Con diferencias", len(con_diferencias), "ti-scale", "#FBF1DC", "#7A5A0E",
         "Montos o comisiones que no coinciden"),
    ]
    _filas_html = "".join(_fila_cruce_categoria(*cat) for cat in categorias)
    _tabla_html = (
        '<table style="border-collapse:collapse;width:100%">'
        '<thead><tr>'
        f'<th style="padding:8px 14px;background:#eef3f8;font-size:10px;font-weight:600;'
        f'color:{_HDR_COLOR};text-transform:uppercase;text-align:left">Categoría</th>'
        f'<th style="padding:8px 14px;background:#eef3f8;font-size:10px;font-weight:600;'
        f'color:{_HDR_COLOR};text-transform:uppercase;text-align:right">Cantidad</th>'
        f'<th style="padding:8px 14px;background:#eef3f8;font-size:10px;font-weight:600;'
        f'color:{_HDR_COLOR};text-transform:uppercase;text-align:left">Descripción</th>'
        f'<th style="padding:8px 14px;background:#eef3f8;font-size:10px;font-weight:600;'
        f'color:{_HDR_COLOR};text-transform:uppercase;text-align:center">Estado</th>'
        '</tr></thead>'
        f'<tbody>{_filas_html}</tbody>'
        '</table>'
    )

    # --- Cross-mes: ventas fuera del período de facturación ML ---
    _cross_mes_html = ""
    if solo_bd:
        try:
            ...
            mes_txt = f"ML las facturará en el próximo Reporte ({mes_siguiente}). No es un error, es información."
        except ValueError:
            rango_txt = "ventas fuera del período de facturación ML"
            mes_txt = "ML las facturará en el próximo Reporte. No es un error, es información."

        _cross_mes_html = (
            '<div style="padding:10px 14px;background:#F7F7F7;display:flex;align-items:center;'
            'gap:12px;border-top:0.5px solid #D8D8D8">'
            '<span style="width:22px;height:22px;flex-shrink:0;border-radius:4px;'
            'background:#E8F1FA;color:#185FA5;display:inline-flex;align-items:center;'
            'justify-content:center"><i class="ti ti-clock" style="font-size:12px"></i></span>'
            '<span>'
            f'<span style="font-weight:700;font-size:16px">{_ar_num(len(solo_bd))}</span>'
            f'<span style="font-size:12px;color:#555"> {rango_txt}</span>'
            f'<br><span style="font-size:10px;color:#9E9E9E">{mes_txt}</span>'
            '</span></div>'
        )
...
def _render_seccion_cruce_ventas(resultado: dict) -> None:
    ...
    with ui.row().classes("w-full justify-end").style("padding:10px 0 16px"):
        ui.button(
